...
### 검출 성능 개선 (바닥 배치 → 선반 배치)

**증상**: 재고를 바닥에 놓고 추론했을 때(버전 1) 검출 성능이 기대에 못 미침

**데이터 현황 (버전 1)**: 총 518장 (Train 363 / Valid 104 / Test 51, 7:2:1), Auto-Orient 적용 + 512×512 Resize(Stretch), Augmentation 미적용

**원인 분석**: 오탐·과적합 문제는 아니라고 판단. valid셋 클래스 분포(cola 35 / gum 38 / snack 31)는 고르게 나왔고, 빛·그림자·물체 각도 등 환경적 변수가 원인일 가능성으로 결론

**해결**: 선반 위에 재고를 올려 각도·배치를 다양화한 92장을 추가 라벨링해 재학습. 데이터셋을 총 610장(Train 427 / Valid 122 / Test 61)으로 갱신

**결과 비교**:

| | 버전 1 | 버전 2 |
|---|---|---|
| Valid 데이터 | cola 35 / gum 38 / snack 31 (104장) | cola 40 / gum 43 / snack 39 (122장) |
| mAP50-95 | 0.96365 | 0.95328 |

mAP50-95는 IoU 임계값 0.5~0.95 전체 평균이라 박스 위치 정밀도를 더 엄격하게 반영하는 지표라, 데이터의 각도·조명·그림자 다양성을 확보하는 과정에서 수치는 다소 낮아졌음. 다만 선반 환경에서 재검증한 결과 9개 물체 전부 검출 성공, 최저 confidence 0.86 / 평균 confidence 0.93으로 실제 사용 환경 기준 검출력은 오히려 개선됨을 확인.
