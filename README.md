...
> 예: 데이터셋 라벨링, 모델 학습 / 서버-클라이언트 통신, 웹 대시보드 등으로 구체적으로 나눠서 적으면 좋습니다.
...
## 성능 및 제약사항
> 실측값으로 채워 넣으면 프로젝트 신뢰도가 크게 올라갑니다.

| 항목 | 값 |
|---|---|
| 평균 추론 FPS | (예: 서버 GPU 기준 OO FPS) |
| 클래스별 mAP@0.5 | (예: 콜라 OO%, 껌 OO%, 과자 OO%) |
| 네트워크 지연 | (예: 로컬 와이파이 기준 약 OOms) |
| 학습 데이터셋 규모 | 클래스당 이미지 수 등 |
